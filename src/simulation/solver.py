...
class SWESolver:
    ...
    def step(self):
        t = min(1.0, self._step_count / self._ramp_steps)
        ramp = float(0.5 * (1.0 - math.cos(math.pi * t)))
        self._step_count += 1

        self._lax_friedrichs_step(ramp)
        self._apply_limiters()
        self._apply_bc_inflow(ramp)
    ...
    def get_frame_data(self) -> dict:
        ...
        return {
            "h": self.h.to_numpy(),
            "eta": self.h.to_numpy() + self.b.to_numpy(),
            "vx": self.vx.to_numpy(),
            "vy": self.vy.to_numpy(),
    ...
    @ti.kernel
    def _lax_friedrichs_step(self, ramp: ti.f32):
        g = self.g
        dx = self.dx
        dy = self.dy
        dt = self.dt
        ...
        for i, j in self.h:
            ...
            # Top/bottom sponge only damps lateral reflection.
            sponge_w = ti.max(ny // 16, 8)
            r_tb = 0.0
            if j < sponge_w:
                r_tb = ti.cast(sponge_w - j, ti.f32) / ti.cast(sponge_w, ti.f32)
            elif j >= ny - 1 - sponge_w:
                r_tb = ti.cast(j - (ny - 1 - sponge_w), ti.f32) / ti.cast(sponge_w, ti.f32)
            if r_tb > 0.0:
                r_tb = r_tb * r_tb * 0.35
                h_val = (1.0 - r_tb) * h_val + r_tb * h0
                hv_val = (1.0 - r_tb) * hv_val

            # Downstream sponge should only absorb outgoing momentum.
            # Do not force h or hu toward inlet values, or it creates a second wave source.
            sponge_r = ti.max(nx // 16, 12)
            if i >= nx - 1 - sponge_r:
                r_out = ti.cast(i - (nx - 1 - sponge_r), ti.f32) / ti.cast(sponge_r, ti.f32)
                r_out = r_out * r_out * 0.55
                hu_val = (1.0 - r_out) * hu_val
                hv_val = (1.0 - r_out) * hv_val
    ...
    def _apply_bc_outflow(self):
        ...
        for j in range(self.ny):
            ...
            self.hu_new[nx - 1, j] = ti.max(0.0, self.hu_new[nx - 2, j])
            self.hv_new[nx - 1, j] = 0.98 * self.hv_new[nx - 2, j]
